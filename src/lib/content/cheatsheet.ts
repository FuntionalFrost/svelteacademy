// src/lib/content/cheatsheet.ts

export type CheatsheetCategory = 'Runes' | 'Props & Binding' | 'Effects & Utilities';
...
<button onclick={() => user.score++}>
  Score: {user.score}
		...
		notes: 'Deeply reactive proxy by default. Replaces Svelte 3/4 `let x = 0;` and writable stores.'
	},
	{
		id: 'state-raw',
		name: '$state.raw',
...
<p>{count} x 2 = {double} ({isEven ? 'Even' : 'Odd'})</p>`,
		notes: 'Replaces Svelte 3/4 `$:` reactive declarations. Derived values are pure and read-only.'
	},
	{
		id: 'derived-by',
		name: '$derived.by',
  ...
  let totalWithTax = $derived.by(() => {
    ...
    return sum * 1.2;
		...
		notes: 'Use when derivation requires loops, branch conditionals, or temporary local variables.'
	},
	{
		id: 'props',
		name: '$props',
...
<input bind:value={value} />`,
  ...
  $effect.pre(() => {
	...
	},
	{
		...
		name: '$inspect',
		category: 'Effects & Utilities',
		summary: 'Console-logs reactive signal changes in development builds. Stripped in production.',
		syntax: '$inspect(count, user).with(console.trace);',
		example: `<script lang="ts">
  let count = $state(0);
  $inspect(count);
</script>`,
		notes: 'Development debugging rune. Attach `.with(fn)` to pipe inspection into custom loggers.'
	},
	{
		id: 'untrack',
		name: 'untrack()',
  ...
  $effect(() => {
    ...
    console.log('Current snapshot of B:', untrack(() => b));
		...
		notes: 'Prevents infinite effect loops when reading auxiliary state values inside side effects.'
	}
];
