...
<div class="my-6 rounded-2xl border border-border bg-card p-6 shadow-xl">
	...
	<div class="flex items-center justify-between border-b border-border/80 pb-4">
		...
		<button
			type="button"
			onclick={reset}
			class="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-1.5 text-sm font-semibold text-muted-foreground transition hover:border-primary/40 hover:text-foreground"
		>
	...
	<div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
		...
		<div class="space-y-4 rounded-xl border border-border bg-muted/20 p-4">
			<div class="flex items-center justify-between">
				<span class="font-mono text-sm font-bold tracking-wider text-cyan-400 uppercase"
			...
			<div class="space-y-2 rounded-lg border border-border bg-background p-3.5">
				...
				<div class="flex gap-2">
					<button
						type="button"
						onclick={() => (count -= 1)}
						class="flex-1 rounded-md border border-border bg-muted/50 py-1.5 font-mono text-sm font-bold text-foreground transition hover:border-primary/40 hover:bg-muted active:scale-95"
					>
						-1
					</button>
					<button
						type="button"
						onclick={() => (count += 1)}
						class="flex-1 rounded-md bg-primary py-1.5 font-mono text-sm font-bold text-primary-foreground shadow-xs transition hover:bg-primary/90 active:scale-95"
					>
						+1
					</button>
				</div>
		...
		<div class="space-y-4 rounded-xl border border-border bg-muted/20 p-4">
			<div class="flex items-center justify-between">
				<span class="font-mono text-sm font-bold tracking-wider text-emerald-400 uppercase"
			...
			<!-- isEven Output -->
			<div
				class="flex items-center justify-between rounded-lg border border-border bg-background p-3.5 text-sm"
			>
				<span class="font-mono text-muted-foreground">isEven ($derived)</span>
				<span
					class={`rounded-md px-2.5 py-0.5 font-mono text-sm font-bold ${isEven ? 'border border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border border-amber-500/30 bg-amber-500/10 text-amber-400'}`}
				>
					{isEven ? 'true' : 'false'}
				</span>
			</div>
		...
		<div class="flex flex-col rounded-xl border border-border bg-muted/20 p-4">
			<div class="mb-3 flex items-center justify-between">
				<span class="font-mono text-sm font-bold tracking-wider text-purple-400 uppercase"
			...
			<div class="flex-1 overflow-hidden rounded-lg border border-border bg-background p-3">
				{#if logs.length === 0}
					<p class="py-4 text-center font-mono text-sm text-muted-foreground">
				...
				{:else}
					<div class="max-h-55 space-y-2 overflow-y-auto pr-1">
						{#each logs as log (log.id)}
							<div
								class="border-b border-border/60 pb-1.5 font-mono text-sm leading-normal last:border-none"
							>
								<span class="block text-sm text-muted-foreground">{log.time}</span>
